...
仓库地址：[lzh-123-ai/Travel-Expense-Compliance-Assistant](https://github.com/lzh-123-ai/Travel-Expense-Compliance-Assistant)
...
文档上传和文档处理是两个显式步骤：上传只负责校验和保存文件，调用 `process` 后才解析、切片；扫描版 PDF 还需单独调用 `ocr`，普通解析链路不会自动加载 OCR。
...
- 使用 PostgreSQL 同时承载业务数据、pgvector 和 GIN 关键词索引，减少引入 Elasticsearch 或独立向量数据库后的运维成本；检索规模扩大后可再拆分服务。
- 线上默认使用 Hybrid+RRF。查询重写和候选重排序保留为可回退实验能力，只有在离线评测证明稳定收益后才适合开启。
- OCR 采用显式触发和逐页质量门槛。低置信度文本保留原文件和待处理状态，不直接写入可检索索引。
